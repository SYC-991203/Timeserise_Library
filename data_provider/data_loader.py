...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path))
        # 你把border理解成一个三元组，broder1s=[0,一年-seq-len，一年零四个月-seq-len]
        # border2s呢是[一年，一年零四个月，一年零八个月]
        # 所以训练集就是一年，测试集就是四个月+seq-len，验证集也是四个月+seq-len

        border1s = [0, 12 * 30 * 24 - self.seq_len, 12 * 30 * 24 + 4 * 30 * 24 - self.seq_len] 
        border2s = [12 * 30 * 24, 12 * 30 * 24 + 4 * 30 * 24, 12 * 30 * 24 + 8 * 30 * 24]
        border1 = border1s[self.set_type] ##border1[0],border2[0]表示训练的起始位置和结束位置，通过train，test和vali的map选出来哪项对应哪项
        border2 = border2s[self.set_type]
