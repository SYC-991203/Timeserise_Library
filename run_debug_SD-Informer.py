...
def run_model(model, data, features, seq_len, label_len, pred_len, e_layers, d_layers, \
              ...
              devices, target, direction,moe_logits_init_dict,loss): # <--- 添加字典参数
    
    # 步骤 1: 将传入的 Python 字典转换为 JSON 字符串
...
# 这个脚本跑llm based embedding模型：SD_Informer,TimeMKG
run_model(
    'TimeMKG', 'DYG_data_3_sub2.csv', features='M', seq_len=96, label_len=48, pred_len=96, e_layers=2, 
    d_layers=1, factor=3, enc_in=5, dec_in=5, c_out=5, d_model=512, d_ff=512, top_k=5, \
          des='test', batch_size=1024, itr=1, devices='0,1,2,3', target='zs',direction='1,1,2,0,0',
          moe_logits_init_dict=moe_init_params,loss='MSE'
          )
