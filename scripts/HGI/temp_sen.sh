    ...
    do
        ...
        do
            ...
            do
                ...
                log_file="./logs/HGI/temp_sen/${model}_${dataset}_${pred_len}_${target}_temp${temp}.log"
                
                # 2. 修改 model_id：必须包含 temp，否则 Checkpoint 会被覆盖！
                # 格式示例: kdd_sub2_96_t5
                current_model_id="${base_model_id}_${pred_len}_t${temp}"

                echo "Running: Model=$model, Dataset=$dataset, Pred_len=$pred_len, Temp=$temp"
...
# 等待所有任务完成
wait
echo "All jobs completed. Logs are saved in ./logs/HGI/temp_sen directory."