...
# 创建日志文件夹
mkdir -p logs/HGI/halluc_rand_score

# 模型列表 (根据你的新代码，使用了 TLMMoeED，你可以根据需要改回 TimesNet 或保留这个)
# models=("HGI" "iTransformer") 
models=("HGI")

# 数据集列表
# datasets=("sub2" "sub3" "sub1")
datasets=("sub2" "sub1" "sub3")
...
do
    ...
    do
        ...
        do
            # 定义日志文件路径
            log_file="./logs/HGI/halluc_rand_score/${model}_${dataset}_${pred_len}_${target}_hrs.log"
