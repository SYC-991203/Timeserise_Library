...
do
    ...
    for model in "${models[@]}"
    do
        # 新的内层循环，遍历不同的预测长度（seq_len）
        for seq_len in "${seq_lens[@]}"
        do
            # 定义日志文件路径
            log_file="./logs/HGI/L_sen/${model}_${dataset}_${seq_len}_${target}.log"
            
            echo "Running: Model=$model, seq_len=$seq_len"
