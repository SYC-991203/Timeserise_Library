...
# exp_groups=("atomization" "global_mixing")
exp_groups=("global_mixing")
...
atom_moe='{"0": [5.0, -5.0], "1": [5.0, -5.0], "2": [5.0, -5.0], "3": [5.0, -5.0], "4": [5.0, -5.0]}'
...
do
    ...
    do
        # 根据实验组选择参数
        if [ "$exp_group" == "atomization" ]; then
            direction="$atom_direction"
            moe_json="$atom_moe"
        elif [ "$exp_group" == "global_mixing" ]; then
