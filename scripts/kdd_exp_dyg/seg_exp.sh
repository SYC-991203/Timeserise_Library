    ...
    for seq_len in "${seq_lens[@]}"; do
        for pred_len in "${pred_lens[@]}"; do
            for model in "${models[@]}"; do
                model_id="${model_id_base}_${model}_seg_L${seq_len}_P${pred_len}"

                echo "Launching $model with seq_len=$seq_len, pred_len=$pred_len for $dataset"
